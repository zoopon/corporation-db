...
このプロジェクトは、大容量データ（3.6百万行以上）を4GBメモリ環境で効率的に処理するためのメモリ最適化を実装しています。

### 最適化の特徴
- **ストリーミング処理**: CSVデータを全てメモリに読み込まず、バッチ単位で処理
- **小さなバッチサイズ**: 500件ずつの処理でメモリ使用量を最小化
- **即座にDB書き込み**: メモリに蓄積せず、読み込み後すぐにデータベースに書き込み

### メモリ使用量
- **従来**: 全データをメモリ保持（メモリ不足でフリーズ）
- **最適化後**: 最大500件分のみメモリ保持（99%以上メモリ削減）

詳細は **[MEMORY_OPTIMIZATION.md](MEMORY_OPTIMIZATION.md)** を参照してください。
