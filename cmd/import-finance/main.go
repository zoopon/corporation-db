...
func main() {
	...
	log.Println("Starting gBizINFO Finance Data Import")
	...
	err = financeUsecase.ImportFromZIPFile(ctx, *inputPath, func(stage string, progress float64) {
	...
	log.Printf("Finance data import completed successfully in %v", duration)

	// Display import statistics
	stats, err := financeUsecase.GetImportStats(ctx)
	if err != nil {
		log.Printf("Warning: Failed to get import stats: %v", err)
	} else {
		log.Printf("Import Statistics:")
		log.Printf("  Total finance records: %d", stats.TotalRecords)
		log.Printf("  Processing time: %v", duration)
		log.Printf("  Records per second: %.2f", float64(stats.TotalRecords)/duration.Seconds())
	}
}
...
func showHelp() {
	fmt.Println(`gBizINFO Finance Data Import Tool

This tool imports finance information from a gBizINFO finance ZIP file
into the local PostgreSQL database.

Usage:
  import-finance -input <path> [options]
...
Examples:
  # Basic import
  ./import-finance -input ./finance_data.zip

  # With custom database settings
  ./import-finance -input ./finance_data.zip -db-host mydb.com -db-user admin

  # Using environment variables
  DB_HOST=production.db.com DB_USER=admin ./import-finance -input ./finance_data.zip

  # Dry run to test configuration
  ./import-finance -input ./finance_data.zip -dry-run

The tool will:
1. Extract and parse the finance CSV data from the ZIP file
